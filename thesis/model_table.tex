...
\begin{itemize}
	\item \textbf{Mesh increase}: A mesh increase enabled the path planning success
	\item \textbf{Disabled corner check}: Path generation was able to proceed after disabling the duplicate corner check.
	\item \textbf{Watershed error}: Watershed repeatedly produced a composite mesh section, even as the merge threshold approached 0, causing an error.
	\item \textbf{Narrow region}: A narrow region was determined to be the root cause of a Watershed error
	\item \textbf{Invalid corner}: A corner was created with only two adjacent regions.
	\item \textbf{Cylinder}: Model contains a cylinder or other complete round edge, which is unable to be processed by \textit{Geometry Simplification}
	\item \textbf{Soft edge}: The model contains a smooth transition from one primitive type to another.
	\item \textbf{Out-of-scope geometry}: The model contains geometry that is beyond the scope of this project, such as swept curves.
\end{itemize}
