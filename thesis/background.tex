...
% This diagram is used twice, thus it makes sense to define it as a pic
...
\begin{equation}\label{eq:2nd_fundamental_tensor}
	\begin{bmatrix}
		\textbf{II}(X_1, X_1) & \textbf{II}(X_1, X_2) \\
...
\begin{tikzpicture}[
	...
	% The ($(A)+(B)$ syntax is used to add / subtract coordinates
...
\begin{tikzpicture}[scale=1.1]
	...
	% One-liner to create the hexagon
	...
	% Inner angles
	% +(A) syntax is non-stacking offset from the previous position.
	% "non-stacking" meaning that additional offsets are based on the previous coordinate,
...
\begin{tikzpicture}[scale=1.0]
	...
	\node[cylinder,shape border rotate=90,
		aspect=2.5, minimum height=20mm, minimum width=20mm, draw=black] at (0,0.0) {};
	% Point
...
% algorithm creates the border / float styling
% algorithmic is the actual pseudocode envirionment
\begin{algorithm}[htb]
