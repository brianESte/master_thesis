% !TeX program = lualatex
% TeX TXS-program:compile = txs:///lualatex/[-shell-escape]
%% Document based on `DEMO-TUDaThesis.tex' version 3.28 (2022/11/15),
%% A part of
%% TUDa-CI -- Corporate Design for TU Darmstadt
%%

\documentclass[
	ngerman,
	ruledheaders=section,%Ebene bis zu der die Überschriften mit Linien abgetrennt werden, vgl. DEMO-TUDaPub
	class=report,% Basisdokumentenklasse. Wählt die Korrespondierende KOMA-Script Klasse
	thesis={type=master},% Dokumententyp Thesis, für Dissertationen siehe die Demo-Datei DEMO-TUDaPhd
	accentcolor=2c,% Default is 9c, 2c for CCPS / IAT
	custommargins=true,% Ränder werden mithilfe von typearea automatisch berechnet
	marginpar=false,% Kopfzeile und Fußzeile erstrecken sich nicht über die Randnotizspalte
	BCOR=0mm,%Bindekorrektur. auf 10mm ändern, falls Druck und Bindung
	parskip=half-,%Absatzkennzeichnung durch Abstand vgl. KOMA-Script
	fontsize=11pt,%Basisschriftgröße laut Corporate Design ist mit 9pt häufig zu klein
	oneside,% Auf "twoside" ändern, falls Druck unf Bindung
	% logofile=example-image, %Falls die Logo Dateien nicht vorliegen
	% captions=oneline,
	% ^ Tried to use that, but the equations in captions in the background caused errors
	numbers=noendperiod,
	% fleqn,
	% NOTE:^ Honestly, having all of the equations pushed left looked a bit ugly,
	% so this option was commented out
]{tudapub}


% Der folgende Block ist nur bei pdfTeX auf Versionen vor April 2018 notwendig
\usepackage{iftex}
\ifPDFTeX
	\usepackage[utf8]{inputenc}%kompatibilität mit TeX Versionen vor April 2018
\fi
% For dealing with SVG issues
% \usepackage{ifluatex}
% \ifluatex
% 	\usepackage{pdftexcmds}
% 	\makeatletter
% 	\let\pdfstrcmp\pdf@strcmp
% 	\let\pdffilemoddate\pdf@filemoddate
% 	\makeatother
% \fi
% \usepackage{svg}

\usepackage{tikz}
\usetikzlibrary{
	arrows.meta,
	calc,
	fit,
	intersections,
	matrix,
	patterns,
	positioning,
	shapes.geometric,
	ext.paths.ortho
}
\usepackage{pgfplots}
\pgfplotsset{width=7cm,compat=1.15}
% "compat" sets the PGFplots compatibility, effectively setting a PGF version to use
\pgfplotsset{
% Define the "no axes" style, for plots without axes
	no axes/.style={
		axis lines=none,
		anchor=origin,
		enlargelimits=false,
		scale mode=scale uniformly}
}
% For plotting a polygon and the boustrophedon path planned within it
% Really only 2 colors are necessary, black for the outline, and red or another color for the path
\pgfplotscreateplotcyclelist{bous-path-2D}{
	black,thick,mark=none\\
	red\\
	green\\
	blue\\
}
% Here multiple colors are needed because all the paths from a given model are shown together
\pgfplotscreateplotcyclelist{bous-path-3D}{
	{black,very thin},
	{red,very thin},
	{green,very thin},
	{blue,very thin},
	{teal,very thin},
	{orange,very thin},
	{cyan,very thin},
	{yellow!60!black,very thin}%
}
% Define global styles
\tikzset{
	% FlowChart node
	FC-Node/.style={rectangle,semithick,align=center,rounded corners=5pt,draw},
	% FlowChart arrow
	FC-Arrow/.style={-{Stealth[round]},semithick,rounded corners=4pt},
	% general vertex
	vertex/.style={circle, radius=2pt,fill, inner sep=2pt,outer sep=0pt},
	% Waypoint2D to mark specific waypoints in bpath graphics
	waypoint-2D/.style={fill=red,circle,outer sep=0pt,inner sep=1.0pt},
	% "vine"(/branch) from the Interior Edge Extension step: PolygonGraph
	Vine/.style={-Stealth}
}
% for multifigure environments
\usepackage{subcaption}
\setcapindent{1.5em}
% \usepackage{svg}

% Pseudocode
\usepackage{algpseudocode}
\usepackage{algorithm}
% Log snippets / actual code
\usepackage{listings}

%%%%%%%%%%%%%%%%%%%
%Sprachanpassung & Verbesserte Trennregeln
%%%%%%%%%%%%%%%%%%%
\usepackage[main=english, ngerman]{babel}
\usepackage[autostyle]{csquotes}% Anführungszeichen vereinfacht

% Falls mit pdflatex kompiliert wird, wird microtype automatisch geladen, in diesem Fall muss diese Zeile entfernt werden, und falls weiter Optionen hinzugefügt werden sollen, muss dies über
% \PassOptionsToPackage{Optionen}{microtype}
% vor \documentclass hinzugefügt werden.
\usepackage{microtype}

%%%%%%%%%%%%%%%%%%%
%Literaturverzeichnis
%%%%%%%%%%%%%%%%%%%
\usepackage[
	backend=bibtex,
	% style=ieee,
	bibencoding=utf8
	% Year (descending) -> Name -> Title
	% sorting=ydnt
]{biblatex}   % Literaturverzeichnis
\addbibresource{references.bib}
%\bibliography{references}

%%%%%%%%%%%%%%%%%%%
%Paketvorschläge Tabellen
%%%%%%%%%%%%%%%%%%%
%\usepackage{array}     % Basispaket für Tabellenkonfiguration, wird von den folgenden automatisch geladen
\usepackage{tabularx}   % Tabellen, die sich automatisch der Breite anpassen
\usepackage{longtable} % Mehrseitige Tabellen
%\usepackage{xltabular} % Mehrseitige Tabellen mit anpassbarer Breite
\usepackage{booktabs}   % Verbesserte Möglichkeiten für Tabellenlayout über horizontale Linien
\usepackage{csvsimple}
% ^ to parse CSV for tables
\usepackage{pdfpages}
% ^ to include the scanned affidavit

% In order to quickly find overfull hboxes
\overfullrule=2mm

%%%%%%%%%%%%%%%%%%%
%Paketvorschläge Mathematik
%%%%%%%%%%%%%%%%%%%
\usepackage{mathtools} % erweiterte Fassung von amsmath
\usepackage{amssymb}   % erweiterter Zeichensatz
%\usepackage{siunitx}   % Einheiten
% To 0-pad numbers. Used to help generate the results graphics directly from OBJ files
\newcommand\TwoDigits[1]{%
	\ifnum#1<10 0#1\else #1\fi
}

\usepackage{pifont}% Zapf-Dingbats Symbole
\newcommand*{\FeatureTrue}{\ding{52}}
\newcommand*{\FeatureFalse}{\ding{56}}

% \addTitleBoxLogo*{\includegraphics[width=0.65\linewidth]{../resources/logos/CCPS_logo.pdf}}
\addTitleBox{\includegraphics[width=\linewidth]{../resources/logos/ccpslogo_landscape.pdf}}
\addTitleBox{
	\includegraphics[width=\linewidth]{../resources/logos/igdLogo.png}
	% \resizebox{\linewidth}{!}{
	% 	\includestandalone{../resources/logos/igd_logo}
	% }
}
%\addTitleBoxLogo*{\includesvg[width=\linewidth]{igd_logo}}

\begin{document}

\Metadata{
	title=Coverage Path Planning on Arbitrary Geometry via Surface Segmentation and Simplification,
	author=Brian Stephenson
}

\title{Coverage Path Planning on Arbitrary Geometry via Surface Segmentation and Simplification}
% \subtitle{\LaTeX{} using TU Darmstadt's Corporate Design}
\subtitle{\textmd{Studiengang Elektrotechnik und Informationstechnik}}
\author[B. Stephenson]{Brian Stephenson}%optionales Argument ist die Signatur,
\reviewer*[Examiner,Advisor,Advisor]{Prof. Dr.-Ing. Rolf Findeisen \and Dr.-Ing Eric Lenz \and MSc. Hasan Kutlu}%Gutachter
% \reviewer[Advisor]{Dr.-Ing Eric Lenz \and MSc. Hasan Kutlu}%Gutachter

%Diese Felder werden untereinander auf der Titelseite platziert.
%\department ist eine notwendige Angabe, siehe auch dem Abschnitt `Abweichung von den Vorgaben für die Titelseite'
\department{metro} % Das Kürzel wird automatisch ersetzt und als Studienfach gewählt, siehe Liste der Kürzel im Dokument. -> metro
\institute{Mechatronics}
\group{Fraunhofer IGD}

\submissiondate{\today}
\examdate{18.11.25}

% Hinweis zur Lizenz:
% TUDa-CI verwendet momentan die Lizenz CC BY-NC-ND 2.0 DE als Voreinstellung.
% Die TU Darmstadt hat jedoch die Empfehlung von dieser auf die liberalere
% CC BY 4.0 geändert. Diese erlaubt eine Verwendung bearbeiteter Versionen und
% die kommerzielle Nutzung.
% TUDa-CI wird im nächsten größeren Release ebenfalls diese Anpassung vornehmen.
% Aus diesem Grund wird empfohlen die Lizenz manuell auszuwählen.
%\tuprints{urn=XXXXX,printid=XXXX,year=2022,license=cc-by-4.0}
% To see further information on the license option in English, remove the license= key and pay attention to the warning & help message.

% \dedication{Für alle, die \TeX{} nutzen.}

\frontmatter
\maketitle
% From IAT template
\clearpage
\setcounter{page}{1}

% Aufgabenstellung page removed.
% Probably because the thesis was done externally, and there was no official Aufgabenstellung
\iffalse
\section*{Aufgabenstellung}
Aufgabenstellungstext vom Betreuer
\vspace{0.5cm}
\begin{tabular}{ll}
    Beginn:   & 02.05.25 \\
    Ende:     & 30.10.25 \\
    Seminar:  & 18.11.25 \\
\end{tabular}
\clearpage
\fi

% If there is an affidavit file, use it, otherwise default to the built-in one
\IfFileExists{../resources/affidavit.pdf}{
	% Despite the template having an affidavit option / macro, it was required to
	% print out the official one, sign it, scan it, and include it here.
	\includepdf{../resources/affidavit.pdf}
}{
	\affidavit[affidavit=digital,signature-image={
		\includegraphics[width=0.3\linewidth]{../resources/sig_rotated.png}
	}]
}
% oder \affidavit[digital] falls eine rein digitale Abgabe vorgesehen ist.
% Es gibt mit Version 3.20 die Möglichkeit ein Bild als Signatur einzubinden.
% TUDa-CI kann nicht garantieren, dass dies zulässig ist oder eine eigenhändige Unterschrift ersetzt.
% Dies ist durch Studierende vor der Verwendung abzuklären.
% Die Verwendung funktioniert so:
%\affidavit[signature-image={\includegraphics[width=\width,height=1cm]{example-image}}, <hier können andere Optionen wie z.B. affidavit=digital zusätzlich stehen>]
\include{abstract}

\cleardoublepage
\tableofcontents
\cleardoublepage

\mainmatter
\include{introduction}

\include{related_work}

\include{background}

\include{methodology}

\include{results}

\include{discussion}

\include{conclusion}

\appendix

\include{model_table}

\printbibliography

\end{document}
