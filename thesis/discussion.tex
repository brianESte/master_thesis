...
% I thought about introducing the discussion chapter, but was unsure how to, and eventually decided against it.
...
The derivative of curvature is better suited to producing boundaries at soft edges, but produces ``double'' boundaries at hard edges.
...
\begin{figure}[htb]
	...
	\begin{subfigure}[t]{0.42\textwidth}
		...
		\label{sfig:m7_gradk}
	\end{subfigure}
	\caption{Comparison of the RMS curvature and the curvature gradient magnitude height functions shown on model ``7120369''.}
\end{figure}
...
This means that Watershed will create mesh regions directly on the hard edge due to the double edge produced when using the gradient magnitude as the height function.
...
Black indicates un-indexed vertices, while the other colors indicate a mesh region.
}
...
This facilitated operating on models with hard edges, but made the procedure unsuitable for models with soft edges.

Two possible solutions to this were devised.
The first possible solution would be to create a height function that takes into account both curvature and its derivative.
...
Because the expansion exponent in \textit{Minima Expansion} is fixed throughout operation, an unsuitable value cannot be escaped or overcome the way a high merge exponent can be.
...
Examples of short edges can be seen in Figure~\ref{fig:short_edges} below.

\begin{figure}[htb]
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../resources/geo_simp/1344058_short_edge_cr.png}
		\caption{%
A short edge on model ``1344058'' is shown with equal mesh density as the rest of the model.}
		\label{sfig:1344058_short_edge}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.45\textwidth}
		\centering
		\includegraphics[width=\linewidth]{../resources/geo_simp/1344055_short_edge_cr.png}
		\caption{%
A short edge on model ``1344055'' is shown with increased mesh density.}
		\label{sfig:1344055_short_edge}
	\end{subfigure}
	\caption{%
A comparison of short edges without (a) and with (b) increased mesh density.
The short edges in each image are highlighted in cyan.
}
	\label{fig:short_edges}
\end{figure}
...
Compare the edge highlighted in Figure~\ref{sfig:1344058_short_edge} with the one in Figure~\ref{sfig:1344055_short_edge}.
The first edge can expect to be filtered out, while the second will remain.
...
Though the exact cause of these invalid corners is unknown, it was observed that the regions of poorer segmentation tended to correlate with their occurrence.
\iffalse
...
At the time of writing, the \textit{Interior Edge Extension} part of the program was not completely bug-free: Model ``fc004'' experienced a duplicate vertex during the interior edge extension step, as indicated in Appendix~\ref{app:model_table}.
Despite this, the \textit{Interior Edge Extension} method, typically accomplished what it was designed to do.
...
Hence, the goal of minimizing the number of switchbacks was unnecessary for this work.
...
\begin{figure}[htb]
	...
	\begin{tikzpicture}[scale=1.0]
		...
		\addplot
			table[col sep=comma] {../resources/boustrophedon/ex_2_polygon.csv};
		\addplot
			table[col sep=comma] {../resources/boustrophedon/ex_2_path.csv};
		\end{axis}
	\end{tikzpicture}
...
For semi-convex polygons the best sweep direction may no longer coincide with the polygon's longest edge, and thus it cannot be chosen based on that.
...
Figure~\ref{sfig:isovist} depicts an isovist in a rectilinear polygon.
...
This is illustrated in Figure~\ref{sfig:bous_guard}.
...
An example of a polygon that fails this test is depicted in Figure~\ref{sfig:concave_bous_guard}.
Changing the guard's orientation by $90^{\circ}$ would allow the dotted regions to be mutually inclusive, but would cause the lower alcoves to be mutually exclusive.
...
\begin{tikzpicture}[scale=0.70]
	\draw[fill=gray!50,draw=none] (1,-1) -- (2,-2) -- (3,-2) -- (3,2) -- (1,2) -- (0.5,1) -- (-3,1) -- (-3,-2) -- (-1.5,-1) -- cycle;
	\draw[thick] (1,-1) -- (1,-2) -- (3,-2) -- (3,2) -- (0.5,2) -- (0.5,1) -- (-3,1) -- (-3,-2) -- (-1.5,-2) -- (-1.5,-1) -- cycle;
	\node[vertex] (guard) at (0,0) {};
\end{tikzpicture}
		\caption{%
An isovist (grayed area) in a rectilinear polygon.
% The vantage point is marked with a solid dot, and the visible area therefrom is shown in gray.
}
		\label{sfig:isovist}
	\end{subfigure}
	\hfill
...
\begin{tikzpicture}[scale=0.70]
	\draw[thick] (1,-1) -- (1,-2) -- (3,-2) -- (3,2) -- (0.5,2) -- (0.5,1) -- (-3,1) -- (-3,-2) -- (-1.5,-2) -- (-1.5,-1) -- cycle;
	\node[vertex] (guard) at (-2,0) {};
	\draw[->] (guard) -- +(90:1);
	\draw[->] (guard) -- +(270:2);
	\draw[->,dashed] (guard) -- +(0:5);
	\draw[->,dashed] (guard) -- +(180:1);
\end{tikzpicture}
		\caption{\raggedright Guard movement in a semi-convex polygon.}
		\label{sfig:bous_guard}
	\end{subfigure}
	\hfill
	\begin{subfigure}[t]{0.3\textwidth}
		\centering
\begin{tikzpicture}[scale=0.70]
	% "pattern=" requires the TikZ patterns library
	\draw[pattern=dots,draw=none] (2,-1) rectangle (3,-2);
	\draw[pattern=dots,draw=none] (2,2) rectangle (3,1);
	\draw[thick] (1,-1) -- (1,-2) -- (3,-2) -- (3,-1) -- (2,-1) -- (2,1) -- (3,1) -- (3,2) -- (0.5,2) -- (0.5,1) -- (-3,1) -- (-3,-2) -- (-1.5,-2) -- (-1.5,-1) -- cycle;
		\node[vertex] (guard) at (1.5,0) {};
	\draw[->] (guard) -- +(90:2);
	\draw[->] (guard) -- +(270:2);
	\draw[->,dashed] (guard) -- +(0:0.5);
	\draw[->,dashed] (guard) -- +(180:4.5);
\end{tikzpicture}
		\caption{%
A non-convex rectilinear polygon.
%Boustrophedon paths planned in the dotted regions are mutually exclusive.
}
		\label{sfig:concave_bous_guard}
	\end{subfigure}
	\caption{%
The polygon shown in (a) and (b) is semi-convex, and can be covered with a boustrophedon path.
Whereas the polygon in (c) cannot, as paths planned in the dotted regions are mutually exclusive.
}
\end{figure}
...
\begin{axis}[
	...
	scale only axis,
	width=5.8cm,
	cycle list name=bous-path-2D
]
	\addplot
		table[col sep=comma] {../resources/boustrophedon/ex_3_polygon.csv};
	\addplot
		table[col sep=comma] {../resources/boustrophedon/ex_3_path.csv};
\end{axis}
		\end{tikzpicture}
		\caption{\raggedright Coverage path for the curved concave shape.}
	\end{subfigure}
	\caption{%
...
The boustrophedon path shown in Subfigure (c) illustrates that it is not necessary for the example shape to be segmented.
}
\end{figure}
...
Hence \textit{Interior Edge Extension} is only useful for simple polygons with all straight edges.
