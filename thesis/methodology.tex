...
% TODO: HK wants me to move the algorithm higher?
\begin{algorithm}[htb]
...
The purpose of the \textit{Geometry Simplification} step is to create a simplified form for each mesh region provided by Segmentation 3D.
...
\Function{createSharedEdges}{Surfaces $S$, UV-maps $M$}
	...
	\While{$P_{remaining}$ not empty}\label{alg:shared_edges:while_k_remaining}
		...
		\State new LineTip queue $K_{tips} \leftarrow p_{edge}$ \Comment{For the beginning of each edge}
...
The \textbf{edge discovery} section (Algorithm~\ref{alg:shared_edges_1}) consists of traversing each mesh section boundary to collect all vertices relevant to that edge and storing them in a list in $E_{points}$.
...
Figure \ref{fig:vertex_graph_init} shows the initial vertex graph created from the example polygon ``blocky bridge''.
The solid black dots are the graph's nodes.
(? Is there any value in giving the example polygon a name?)
...
\begin{tikzpicture}
	% \draw[step=10mm,gray!50,very thin] (-0.2,-0.2) grid (6.2,3.2);
	\draw (0,0) -- (1,0) -- (2,1) -- (0,1) -- cycle;
	\node at (0.5,0.5) {0};
	\draw (0,1.5) -- (2,1.5) -- (3,2.5) -- (2,3.5) -- (0,3.5) -- cycle;
	\node at (1.0,2.5) {1};
	\draw (2.5,1) -- (4.5,1) -- (3.5,2) -- cycle;
	\node at (3.5,1.5) {2};
	\draw (3.5,3) -- (4.5,4) -- (2.5,4) -- cycle;
	\node at (3.5,3.5) {3};
	\draw (4,2.5) -- (5,1.5) -- (7,1.5) -- (7,3.5) -- (5,3.5) -- cycle;
	\node at (6.0,2.5) {4};
	\draw (5,1) -- (6,0) -- (7,0) -- (7,1) -- cycle;
	\node at (6.5,0.5) {5};
\end{tikzpicture}
