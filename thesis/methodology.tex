%! TeX root = thesis.tex
\chapter{Methodology}\label{sec:mtdy}
\IMRADlabel{methods}
...
% The previous sentence sounds odd with ", such as plane, cylinder, cone, etc." but also sounds off using the plurals...
Next is \textit{Geometry Simplification}, which unwraps each mesh section in order to create an idealized form from it.
\textit{2D Segmentation} is an conditional step, that segments non-convex simplified surfaces into convex ones.
...
In \textit{Inverse Kinematics} the waypoints of the complete path are converted to actuator positions and sent to the connected robotic arm and rotary table to be executed.
...
\begin{tikzpicture}[]
	...
	\node[FC-Node, text width=28mm] (WS) [right=\dx of Mesh] {Watershed Segmentation};
	\node[FC-Node, text width=28mm] (SurfCl) [right=\dx of WS] {Surface Classification};
	\node[text width=25mm, align=center] (MRs) [right=\dx of SurfCl] {Classified Mesh Sections};
	% connections
	\draw [FC-Arrow] (Mesh) -- (WS);
	\draw [FC-Arrow] (WS) -- (SurfCl);
	\draw [FC-Arrow] (SurfCl) -- (MRs);
	% \path (SurfCl.south) -- node[below=5mm of SurfCl] (CompPrim) {Composite Primitive} (WS.south);
	% \draw [FC-Arrow, rounded corners=5pt] (SurfCl.south) |- (CompPrim) -| (WS.south);
	\draw [FC-Arrow, rounded corners=5pt] (SurfCl.south) -- +(0,-0.4) -| (WS.south)
		node[pos=0.25,minimum size=16pt,
		label={[label distance=-11pt]270:Composite surfaces}] (CompSurf) {};
	% Segmentation 3D container node
	\node[rectangle,rounded corners=8pt,draw,thick,dashed,inner sep=8pt,fit=(WS)(SurfCl)(CompSurf),label={above:3D Segmentation}] {};
\end{tikzpicture}
...
% They showed that it is possible to segment a 3D mesh using its curvature.
...
% Subfigure b of Atkar et al.'s Figure~\ref{fig:ws_1570179_steps} is deceptively drawn, depicting different expansion depths for the different regions.
...
% NOTE: in the code mini-merge is numbered 4.1, and shallow-merge 4.2, but for simplicity they are 4 and 5 here.
...
Figure~\ref{sfig:ws_2} shows the result of \textit{Minima Expansion} on the test object.
...
% NOTE: the function label was removed to allow both pseudocodes to fit on the same page with text
\begin{algorithmic}[1]
% \Function{miniMerge}{Mesh sections $S$}
	\For{Mesh section $s$ in $S$}
		\If{$s$.small()}
...
\begin{algorithmic}[1]
\Function{MeshRegion.findLowSideNeighbor}{ }
...
\begin{algorithmic}[1]
	\Function{shallowMerge}{Mesh sections $S$, real $e$}
		\State Sort $S$ by mesh section depth in ascending order
		% \State $d_{th} \leftarrow d_{deepest}^{e}$\Comment{See Equation~\ref{eq:merge_threshold} for explanation}
		\State calculate merge threshold $d_{th}$ from $d_{max}$ and $e$ \Comment{See Equation~\eqref{eq:merge_threshold}}
...
% On line 3 the function \verb|shallowMerge|'s second argument $e$ is used in Equation~\eqref{eq:merge_threshold} to calculate the merge threshold $d_{th}$ from
...
% After implementing a conic error function based on point-surface distance, conic regression could be used \textit{as} cylinder classification.
...
\begin{tikzpicture}[inner sep=0]
	% This diagram uses the TikZ matrix library to place the nodes
	% Nodes
	\matrix (table) [
		matrix of nodes,
		...
		% The [6mm] adds extra space between this column and the next
		\node[text width=24mm, align=center] (MRs) {Classified Mesh Sections}; &[6mm]
		...
		\node[text width=20mm, align=center] (SSurfaces) {Simplified Surfaces}; \\
		& & & & \\
	};
...
The purpose of the \textit{Geometry Simplification} step is to create a simplified form for each mesh region provided by \textit{3D Segmentation}.
...
% NOTE: leaning towards bold for structs such as SimpleSurface and SharedEdges
...
\Function{createSharedEdges}{Surfaces $S$, UV-maps $M$}
	...
	\While{$P_{remaining}$ not empty}\label{alg:shared_edges:while_k_remaining}
		\Comment{Checks an edge graph in each iteration}
		\State new point $p_{edge} \leftarrow$ first value in $P_{remaining}$
		\State new EdgeStart queue $K_{start} \leftarrow$ EdgeStart($p_{edge}$) \Comment{For potential EdgeStarts}
		\While{$K_{start}$ not empty}\label{alg:shared_edges:while_k_start}
			\Comment{Creates a edge point list in each iteration}
			...
			\State new queue $P_{search} \leftarrow k_{start}$.pt %\Comment{Queue of points to search along current edge}
			\While{$P_{search}$ not empty}
				...
				\For{$p_{adj}$ in $p_{edge}$.adjacentPoints}
...
Algorithm~\ref{alg:shared_edges_1} presents the \textbf{edge discovery} portion of \textit{Create Shared Edges}.
...
\begin{algorithmic}[1]
	...
	\For{Point list $P_{edge}$ in $E_{points}$}\label{alg:edge_refinement_for}
		\If{Number of points in $P_{edge}$ < $n_{min}$}
			\State \textbf{continue} \Comment{Assume tiny edges are a mistake and skip them}
		\EndIf
...
Algorithm~\ref{alg:shared_edges_2} details the \textbf{edge refinement} process.
...
% NOTE: If this algorithm float is not positioned using a '!', it _will_ end up at the end of the chapter
\begin{algorithm}[!tb]
...
Recall that the points that make up \verb|SharedEdge|s are taken from the mesh.
...
% \verb|CreateSharedCorners()| is primarily a \verb|for| loop
...
\begin{algorithmic}[1]
\Function{clampLineTips}{SharedCorner list $C$, UV-maps $M_{UV}$}
	\For{$c$ in $C$}
		...
		\For{line tip $t$ in $c$.lineTips()}
			\State new set $S_{edge} \leftarrow t$.adjacentRegions() %$E$[$t$].adjacentRegions()
				\Comment{The surfaces adjacent to $t$'s edge}
...
% NOTE: This float also requires its '!' for proper positioning
\begin{algorithm}[!htb]
...
Hence the \verb|if-else| statement on lines 15-18.
...
\begin{enumerate}
	...
	\item The completed vertex graph is then traversed to form a polygon graph, where each node is an atomic convex polygon.
		% NOTE: thinking "minimal" might be a better descriptor than "atomic"
...
\caption{Example polygon to help illustrate \textit{Interior Edge Extension} with corners numbered.}
	% \label{fig:extend_shared_edges}
\end{figure}
...
Figure~\ref{fig:vertex_graph_init} shows the initial vertex graph created from the example polygon.
The solid black dots are the graph's nodes.
% NOTE: Thinking of moving this figure next to the figure above... there is hardly any difference between the 2 figures...
\begin{figure}[htb]
	\centering
\begin{tikzpicture}
	\draw[step=10mm,gray!50,very thin] (-0.2,-0.2) grid (6.2,3.2);
...
\end{tikzpicture}
\caption{Initial vertex graph created from the example polygon}
...
\begin{figure}[htb]
	...
	\begin{subfigure}[t]{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=0.94]
			\draw[step=10mm,gray!50,very thin] (0.0,0.0) grid (6.0,3.0);
			\draw[very thick] (0,0) node (0) [vertex,label=225:0] {} --
				(1,0) node (1) [vertex,label=287.5:1] {} --
				(2,1) node (2) [vertex,label=287.5:2] {} --
				(4,1) node (3) [vertex,label=247.5:3] {} --
				(5,0) node (4) [vertex,label=247.5:4] {} --
...
Next the algorithm iterates through the edge loop's vertex nodes, with exterior corners skipped.
...
\Function{PolygonGraph::create}{}
	...
	\While{$B_{active}$ not empty}
		...
		\If{$b_{active}$ in $B_{mature}$ or $B_{complete}$}
			\State \textbf{continue}
		\EndIf
		\State check for intersection between $b_{active}$ and a branch in $B_{mature}$\label{alg:l:b_active_intersection}
		...
		\ElsIf{$b_{active}$.tip has 3 or more edges} \Comment{Create new active branches}
...
\begin{figure}[htb]
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=0.94]
			...
			\draw[very thick] (0,0) node (N0) [vertex,label=225:0] {} --
				(1,0) node (N1) [vertex,label=287.5:1] {} --
				(2,1) node (N2) [vertex,label=287.5:2] {} --
				(4,1) node (N3) [vertex,label=247.5:3] {} --
				(5,0) node (N4) [vertex,label=247.5:4] {} --
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=0.94]
			...
			\draw[very thick] (N3) -- (N11) -- (N12);
			% Draw vines / branches
			% To create well-aligned lines for the branches,
			% many coordinates and intersections were used
			% See https://texample.net/intersection-of/ for a simpler example
			\coordinate (N0o) at ($(N0)+(45:0.15)$);
			\coordinate (N1o) at ($(N1)+(112.5:0.15)$);
			\coordinate (N2o) at ($(N2)+(202.5:1)$);
			\path[name path=N1o](N1) -- (N1o);
...
\begin{tikzpicture}[every node/.style={font=\Large}]
	% Draw complete vertex graph
	\draw[very thick,every node/.style={vertex,font=\small}]
		(2,3) node (N12)[vertex,label=90:12] {} --
		(3,2) node (N11)[vertex,label= 0:11] {} --
		(4,1) node (N3) [vertex,label=247.5:3] {} --
		(5,0) node (N4) [vertex,label=247.5:4] {} --
		(6,0) node (N5) [vertex,label=315:5] {} --
		(6,1) node (N10)[vertex,label= 0:10] {} --
		(6,3) node (N6) [vertex,label= 45:6] {} --
		(0,3) node (N7) [vertex,label=135:7] {} --
		(0,1) node (N9) [vertex,label=180:9] {} --
		(0,0) node (N0) [vertex,label=225:0] {} --
		(1,0) node (N1) [vertex,label=287.5:1] {} --
		(2,1) node (N2) [vertex,label=287.5:2] {} --
		(4,3) node (N8) [vertex,label= 90:8] {};
	\draw[very thick] (N9) -- (N10);
	% Polygon node numbers
	\node at (0.7,0.5) {0};
	\node at (1.2,2.0) {3};
	\node at (3.0,1.4) {1};
	\node at (3.0,2.6) {4};
...
\end{tikzpicture}
	\caption{%
The complete polygon graph of the example polygon.
The number in the center of each atomic polygon is its index.
}
...
For example, a CMO comprised solely of node 3 in Figure~\ref{fig:pg_complete} has 3 open edges: bordering node 1 (vertices 2-9), bordering node 3 (vertices 2-11), and bordering node 4 (vertices 11-12).
However, a \{0,3\} CMO has only 1 open edge: bordering node 4.
The edge between nodes 0 and 3 allowed them to merge, and node 3's edge bordering node 1 (vertices 2-11) is no longer considered open and valid because there is no collinear open edge on node 0.
That the edge between the \{0,3\} CMO and \{1\} CMO is closed, prevents them from merging and creating a non-convex merge option.
As another example, when the \{3\} CMO and \{4\} CMO merge, \{3\}'s edge with node 1 and \{4\}'s edge with node 5 also merge to form a single open edge from vertex 2 to 8.
The combined \{3,4\} CMO can no longer merge with the \{1\} or \{5\} CMO, because neither has an open edge from vertex 2 to 8, but \textit{can} merge with the \{1,5\} CMO.
...
% NOTE: a "\ " (escaped space) tells LaTeX to use an interword space.
% Useful when it might otherwise use an intersentence space.
...
For the example presented, there are 15 total CMOs: six from single sub-polygons: \{0\},...,\{5\}, six from sets of two sub-polygons: \{0,3\},...,\{4,5\}, two from sets of 3 sub-polygons: \{2,4,5\}, \{0,3,4\}, and one comprised of 4 sub-polygons: \{1,3,4,5\}.
...
% \end{split}
% \]
% Not really worth displaying this

\subsection{Optimal Polygon Set}%\label{OptimalCMOs}
...
% I do not know at what level it becomes noticably problematic. At 20 it works ok, at 60 it never returns...
...
A Set Cover Problem attempts to determine the minimum subset of $S$ that covers the problem's ``universe''.
...
For example, $h(s)$ for CMO \{0\} is 1, because $n_s$ = 1 and $w_s$ = 1, whereas $h(s)$ for CMO \{1,3,4,5\} is 2, because its $n_s$ = 4, and $w_s$ = 2.

\begin{algorithm}[htb]
\caption{Find approximate solution to SCP using a greedy algorithm}\label{alg:greedySCP}
\begin{algorithmic}[1]
\Function{greedySCP}{CMO list $S$}
	\State new list $S_{sorted} \leftarrow$ sort $S$ by g value
	\State new list $L_{PS}$ \Comment{To store the Polygon Subsets created}
	\State new PS $p_{worst} \leftarrow$ worst case PS \Comment{Used as base of reference}
	\State $L_{PS}$.insert($p_{worst}$)
	\State new int $l_{spawn} \leftarrow 4$ \Comment{Sets the function spawn limit}
	\For{uint $i \leftarrow 0,...,l_{spawn}$}
...
\begin{algorithmic}[1]
\Function{expandPartialPS}{PS list $L_{PS}$, PartialPS $p$, uint $b$, CMO list $S$}
	\If{$p$.complete()}
		\If{$p$.widthSum() < lowest width sum in $L_{PS}$}
			\State $L_{PS}$.insert(PS from $p$)
		\EndIf
		\State \textbf{return}
	\EndIf
	\State int $l_{spawn} \leftarrow 4$
	\For{uint $i \leftarrow b,...,n_S$}
		\State CMO $s \leftarrow$ $S$[$i$]
		\If{$p$.uSetsOverlap($s$.subpolygons())}
			\State \textbf{continue}
		\EndIf
		\If{$p$.widthSum() + $s$.width() > lowest width sum in $L_{PS}$}
			\State \textbf{continue}
		\EndIf
...
% NOTE: the following float seems to only sit where i want it with the !p options...
...
\Function{boustrophedon}{SimplePolygon $P$, sweep direction $s$, shift distance $d_{shift}$}
	...
	\While{$w$.y < $h_{pass}$}
		...
		\State new waypoint $w_{prop} \leftarrow w$ moved up $d_{edge}$ along $e_{curr}$
			\Comment{Proposed waypoint}
		\If{$w_{prop}$.y > $h_{pass}$}
			\State $d_{edge} \leftarrow $($h_{pass} - w$.y) / $e_{curr}$.dir.y
		\ElsIf{$w_{prop}$.y > $c_{curr}$.y}
			\State $w \leftarrow c_{curr}$
			\State $W_{bous}$.append($w$)
			\State $e_{curr}$.next(); $c_{curr}$.next()
			\State $d_{edge} \leftarrow (w_{prop}$.y - $w$.y) / $e_{curr}$.dir.y
		\EndIf
...
\begin{figure}[htb]
	...
	\begin{subfigure}{0.48\textwidth}
		\centering
			...
			\begin{axis}[no axes,cycle list name=bous-path-2D]
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_polygon.csv};
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_perim.csv};
			\end{axis}
		\end{tikzpicture}
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=1.0]
			...
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_polygon.csv};
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_sweep_1.csv};
			% Mark current corner
			\draw (axis cs:-0.614514, 3.89323) circle[radius=2pt]
				node[coordinate,label=right:{$c_{curr}$}] {}; % was pin
			% Mark post-shift waypoint
			\node[waypoint-2D] at (axis cs:-0.284931, 2.71528) {};
			% Mark current direction / edge
			\draw[-{stealth}] (axis cs:-0.111943, 2.09701) -- (axis cs:-0.284931, 2.71528)
				node[right] {$e_{curr}$};
			\end{axis}
		\end{tikzpicture}
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=1.0]
			...
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_polygon.csv};
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_sweep_2.csv};
			% Circle at current corner
			\draw (axis cs:10.6172, 2.88409) circle[radius=2pt]
				node[coordinate,label={above left:{$c_{curr}$}}] {};
			% Mark current direction / edge
			\draw[-{stealth}] (axis cs:10.6, 1.5) -- (axis cs:10.62, 2.75)
				node[below left] {$e_{curr}$};
			\end{axis}
		\end{tikzpicture}
	...
	\begin{subfigure}{0.48\textwidth}
		...
		\begin{tikzpicture}[scale=1.0]
			...
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_polygon.csv};
			\addplot
				table[col sep=comma] {../resources/boustrophedon/ex_1_sweep_3.csv};
			% Circle at current corner
			\draw (axis cs:8.76958, 5.67303) circle[radius=2pt]
				node[coordinate,label={left:{$c_{curr}$}}] {};
			% Mark waypoints
			\node[waypoint-2D] at (axis cs:10.6151, 2.71528) {};
			\node[waypoint-2D] at (axis cs:10.6172, 2.88409) {};
			\node[waypoint-2D] at (axis cs:10.187, 3.51528) {};
			% Mark current direction / edge
			\draw[-{stealth}] (axis cs:10.6172, 2.88409) -- (axis cs:10.187, 3.51528)
				node[below left] {$e_{curr}$};
			\end{axis}
		\end{tikzpicture}
...
The algorithm inputs are a polygon, the ``sweep direction'', and the ``shift distance''.
The sweep direction is the long pass direction (the horizontal direction in Figure~\ref{fig:bpath_steps}), and the shift distance is the perpendicular distance between long passes.
At this point the end effector tool and its characteristics become relevant.
The envisioned tool is a high-powered laser with a set beam width to ablate the surface of the target object.
...
However for a cylindric surface moving up and down the cylinder (an axially oriented sweep direction) would result in uneven laser application, whereas moving \textit{around} the cylinder (a circumferential sweep direction) would ensure the surface that the laser contacts is, relative to the laser beam, flat, and thus an even laser application.

The procedure shown in Algorithm~\ref{alg:boustrophedon} begins by checking if a sweep direction was provided, and if not, creating one from the polygon's longest edge.
...
A temporary waypoint $w_{prop}$ is created at this proposed position to check a couple conditions.
% NOTE: should probably rename h_pass...
First, if $w_{prop}$'s y coordinate is greater than $h_{pass}$, $d_{edge}$ is updated so that the next waypoint $w$ is no higher than $h_{pass}$, as this will be the final shift and sweep pass.
Second, if $w_{prop}$ is above the current corner, then a waypoint is added at the corner and both the current edge and corner are moved up to their next positions.
...
This is to ensure that the intersection of the sweep direction extended from the last waypoint and the current edge is correct.
% This is to ensure that the intersection between the current waypoint along the sweep direction and the current edge is correct.
...
\begin{tikzpicture}[scale=0.9,
	...
	\node[nBalloon] at (-6,4.55) {1}; % Load model
	\node[nBalloon] at (-4,4.55) {2}; % Reload models
	\draw[nbArrow] (-7.55,3.7) node[nBalloon] {3} -- +(350:3.5mm); % Check mesh quality
	% \node[nBalloon] at (-7.5,2.8) {4}; % Start
	% \node[nBalloon] at (-2.5,2.8) {5}; % Stop
	% Path planning controls
	% In order to better control the end points of the "areaLines" a
	% transparent rectangle was placed over the target region, and its corners
	% used as the end points for those lines
	...
	\draw[nbArrow] (-2.4,2.37) node[nBalloon] {5} -- +(180:3.5mm); % Smoothing sigma
	\draw[nbArrow] (-2.4,1.87) node[nBalloon] {6} -- +(180:3.5mm); % Merge threshold
	% YieldPoint window
	% minimum height and width control the height and width of a node
	% useful for setting the size of the node larger than its content
	...
	% Execute path
	\draw[nbArrow] (-2.4,-2.40) node[nBalloon] {8} -- +(180:3.5mm);
	...
	\draw[nbArrow] (-1,-3.9) node[nBalloon, in front of path] {11} -- +(270:4.4mm);
\end{tikzpicture}
...
\begin{enumerate}
	...
	\item \textbf{Yield Point Window}: To facilitate debugging and displaying intermediate results ``Yield Points'' were developed.
		...
		% especially if the \textit{3D Segmentation} yield points are inactive.
...
% NOTE: Is there any merit in describing the unit tests created? -> Apparently not
