...
Given mesh regions $A$ and $B$, a shared edge between $A$ and $B$ is formed from the list of vertices that are either in $A$ and adjacent to $B$ or in $B$ and adjacent to $A$.
...
Obtaining said list of vertices is done by finding a start point on a valid boundary and spreading out along the boundary from there.
		...
		\While{$K_{tips}$ not empty}
			...
			\While{$P_{search}$ not empty}
				...
				\For{$p_{adj}$ in $p_{edge}$.adjacent\_points}
					\If{$p_adj$ on current region boundary}
